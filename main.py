...
# ============= SETTINGS - YAHI CHANGE KARNA =============
SUPPORT_GROUP = "https://t.me/+AAB-iIMnebBmMWZl" # Group link
UPDATE_CHANNEL = "https://t.me/+AAB-iIMnebBmMWZl" # Channel link
OWNER_LINK = "https://t.me/KARTIK_NISHAD_3" # Owner link
BOT_USERNAME = "https://t.me/YourBotUsername" # Add me baby button
...
async def help(client, message: Message):
    text = """🔥 **ISHIKA USERBOT V2.2 FINAL** 🔥

**Tag/Admin:** `/tagall` `/cancel` `/tagsh` `/stoptagsh` `/promote` `/demote` `/ban` `/unban` `/kick` `/warn` `/zombies`
**Info:** `/id` `/info` `/clone` `/back` `/purge`
...
async def clone(client, message: Message):
    ...
    if photos:
        ...
        os.remove(file)
    await client.invoke(UpdateProfile(first_name=user.first_name, last_name=user.last_name or "", bio=user.bio or ""))
    await message.edit(f"✅ Cloned: **{user.first_name}**\nDP + Name + Bio Copied")
...
async def back(client, message: Message):
    ...
    await message.edit("🔄 Restored to original")
...
async def logo_gen(client, message: Message):
    ...
    elif args[0].lower() in ["girl", "g"]: gender, name = "girl", " ".join(args[1:])
    ...
    try:
        prompt = f"anime {gender} character portrait, aesthetic background, glowing, cinematic, high quality"
...
async def tagsh(client, message: Message):
    ...
    shayari_copy = SHAYARI_LIST.copy(); random.shuffle(shayari_copy); i = 0
    async for member in client.get_chat_members(chat_id):
        ...
        if member.user and not member.user.is_bot and not member.user.is_deleted:
            ...
            try: await client.send_message(chat_id, text); i += 1; await asyncio.sleep(4)
            except FloodWait as e: await asyncio.sleep(e.value)
            except: pass
    tagsh_active[chat_id] = False; await client.send_message(chat_id, "✅ **TAGSH COMPLETED**\nSabko tag kar diya")
...
async def broadcast(client, message: Message):
    ...
    async for d in client.get_dialogs():
        ...
        if sent % 10 == 0: await status.edit(f"📢 Sending... \n✅ Sent: {sent}\n❌ Failed: {failed}")
...
async def gcast(client, message: Message):
    ...
    msg = " ".join(message.command[1:]); sent, failed = 0, 0
    status = await message.edit("📢 Group Broadcasting...")
    async for d in client.get_dialogs():
        if d.chat.type in ["group","supergroup"]:
            try: await client.send_message(d.chat.id, f"📢 **GROUP BROADCAST**\n\n{msg}"); sent += 1
            except: failed += 1
            await asyncio.sleep(3)
...
async def dcast(client, message: Message):
    ...
    msg = " ".join(message.command[1:]); sent, failed = 0, 0
    status = await message.edit("📢 DM Broadcasting...")
    async for d in client.get_dialogs():
        if d.chat.type=="private" and not d.chat.is_bot:
            try: await client.send_message(d.chat.id, f"📢 **DM BROADCAST**\n\n{msg}"); sent += 1
            except: failed += 1
            await asyncio.sleep(3)
...
async def tagall(client, message: Message):
    ...
    async for user in client.get_chat_members(message.chat.id):
        ...
        await client.send_message(message.chat.id, f"[{user.user.first_name}](tg://user?id={user.user.id}) {msg}"); await asyncio.sleep(5)
...
async def promote(client, message: Message):
    await client.promote_chat_member(message.chat.id, message.reply_to_message.from_user.id, privileges=ChatPrivileges(can_manage_chat=True,can_delete_messages=True,can_restrict_members=True,can_invite_users=True,can_pin_messages=True)); await message.edit("✅ Promoted")
@app.on_message(filters.me & filters.command("demote"))
async def demote(client, message: Message):
    await client.promote_chat_member(message.chat.id, message.reply_to_message.from_user.id, privileges=ChatPrivileges()); await message.edit("✅ Demoted")
...
print("ISHIKA USERBOT V2.2 FINAL STARTED ✅")
app.run()
